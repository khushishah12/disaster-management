...
import { getActiveRescueReports, getMyRescueReports } from "@/lib/rescue/actions";
...
const REPORT_TYPE_META: Record<string, { label: string; icon: string }> = {
  rescue_operation: { label: "Rescue Op", icon: "🆘" },
  casualty_evacuation: { label: "Evacuation", icon: "🚑" },
  search_operation: { label: "Search", icon: "🔍" },
...
export function RescueTeamDashboard({ profile }: RescueTeamDashboardProps) {
  const [view, setView] = useState<ViewState>("dashboard");
  const [detailId, setDetailId] = useState<string | null>(null);

  const { data: assignments = [] } = useQuery({
    queryKey: ["rescue-team-assignments"],
    queryFn: async () => {
  ...
  const { data: activeReports = [] } = useQuery({
    queryKey: ["rescue-active-reports"],
    queryFn: async () => {
      const res = await getActiveRescueReports();
      return res.data ?? [];
    },
  ...
  const { data: allReports = [] } = useQuery({
    queryKey: ["rescue-all-reports"],
    queryFn: async () => {
      const res = await getMyRescueReports();
      return res.data ?? [];
    },
    staleTime: 60 * 1000,
  });
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-semibold text-slate-100">All Rescue Reports ({allReports.length})</h2>
        </div>
        {allReports.length === 0 ? (
          <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-8 text-center">
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">Rescue Team Dashboard</p>
          </div>
          <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-[11px] font-semibold text-amber-300">Rescue Team</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-3">
          ...
          <p className="mt-1 text-2xl font-bold text-amber-400">{activeAssignments.length}</p>
        </div>
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-3">
      ...
      <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button type="button" onClick={() => setView("all-reports")}
            className="rounded-lg bg-slate-800/40 px-3 py-3 text-center transition hover:bg-slate-800/60">
            <p className="text-lg mb-1">🆘</p>
            <p className="text-xs font-semibold text-slate-300">Rescue Reports</p>
            <p className="text-[10px] text-slate-500 mt-0.5">View all submitted rescue reports</p>
          </button>
        </div>
      </div>
    </div>
