...
import { getMyTeamAssignments, getEmergencyProfile } from "@/lib/sos/actions";
import { getActiveReports, getMyReports } from "@/lib/ambulance/actions";
import type { TeamAssignmentRow } from "@/lib/sos/actions";
...
const REPORT_TYPE_META: Record<string, { label: string; icon: string }> = {
  emergency_incident: { label: "Emergency", icon: "🚑" },
  patient_transfer: { label: "Transfer", icon: "🔄" },
  road_blockage: { label: "Road Block", icon: "🚧" },
  hospital_dropoff: { label: "Drop-off", icon: "🏥" },
  availability: { label: "Available", icon: "✅" },
};
...
export function AmbulanceDashboard({ profile }: AmbulanceDashboardProps) {
  ...
  const { data: activeReports = [] } = useQuery({
    queryKey: ["ambulance-active-reports"],
    queryFn: async () => {
      const res = await getActiveReports();
      return res.data ?? [];
    },
  ...
  const { data: allReports = [] } = useQuery({
    queryKey: ["ambulance-all-reports"],
    queryFn: async () => {
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-semibold text-slate-100">All Reports ({allReports.length})</h2>
        </div>
        {allReports.length === 0 ? (
          <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-8 text-center">
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">Ambulance Team Dashboard</p>
          </div>
          <span className="rounded-full border border-blue-500/30 bg-blue-500/10 px-3 py-1 text-[11px] font-semibold text-blue-300">Ambulance Team</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-3">
          ...
          <p className="mt-1 text-2xl font-bold text-blue-400">{activeAssignments.length}</p>
        </div>
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-3">
      ...
      <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button type="button" onClick={() => setView("all-reports")}
            className="rounded-lg bg-slate-800/40 px-3 py-3 text-center transition hover:bg-slate-800/60">
            <p className="text-lg mb-1">🚑</p>
            <p className="text-xs font-semibold text-slate-300">Ambulance Reports</p>
            <p className="text-[10px] text-slate-500 mt-0.5">View all submitted reports</p>
          </button>
        </div>
      </div>
    </div>
