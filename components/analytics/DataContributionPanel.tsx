...
export const DataContributionPanel = () => {
  ...
  const [drawerKind, setDrawerKind] = useState<"contribution" | "ambulance" | null>(null);
  ...
  const isLoadingAll = isLoading || ambLoading;

  const openDrawer = (kind: "contribution" | "ambulance", id: string) => {
    setDrawerKind(kind);
    setDrawerId(id);
  };
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) =>
            item.kind === "contribution" ? (
              <ContributionCard key={item.data.id} c={item.data} onClick={() => openDrawer("contribution", item.data.id)} />
            ) : (
              <AmbulanceReportCard key={item.data.id} r={item.data} onClick={() => openDrawer("ambulance", item.data.id)} />
            )
          )}
        </div>
      )}
